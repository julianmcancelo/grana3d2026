...
import { ArrowRight, Layers, Box, Truck, Zap, Star, Instagram, MessageCircle, Heart, Loader2, Check, Sparkles } from 'lucide-react'
...
function InstagramFeed() {
   return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         {[1, 2, 3, 4].map((i) => (
            <div key={i} className="aspect-square bg-gray-200 dark:bg-gray-800 rounded-xl animate-pulse flex items-center justify-center text-gray-400 dark:text-gray-600">
               feed placeholder
            </div>
         ))}
      </div>
...
export default function LandingPage() {
   ...
   useEffect(() => {
      const cargarConfig = async () => {
         try {
            ...
            if (data.modoProximamente) {
               setModoProximamente(true)
               setTextoProximamente(data.textoProximamente || '¡Estamos preparando algo increíble!')
            }
            // Guardar datos de contacto
            if (data.whatsapp || data.instagram) {
               setConfig({
                  whatsapp: data.whatsapp?.replace(/\D/g, '') || '5491112345678',
                  instagram: data.instagram?.replace('@', '') || 'grana3d'
               })
            }
   ...
   // Modo Próximamente - pantalla completa elegante
      ...
      <div className="bg-gray-50 dark:bg-black min-h-screen text-gray-900 dark:text-white overflow-x-hidden font-sans selection:bg-teal-500 selection:text-white">
            ...
            <div className="grid md:grid-cols-2 gap-16 items-center">
               <div>
                  <h2 className="text-4xl md:text-6xl font-bold mb-8 leading-tight text-gray-900 dark:text-white">
                     Más que plástico fundido. <span className="text-teal-600 dark:text-teal-500">Ingeniería.</span>
                  </h2>
                  <p className="text-gray-600 dark:text-gray-400 text-lg mb-8 leading-relaxed">
                     No somos una granja de impresoras genérica. Nos enfocamos en la calidad superficial, la resistencia mecánica y la precisión dimensional de cada pieza.
                  </p>

                  <div className="space-y-6">
                     {[
                        { title: 'Resolución Ultra-Fina', desc: 'Capas de hasta 0.05mm para detalles invisibles.' },
                        { title: 'Materiales Técnicos', desc: 'Desde PLA decorativo hasta Nylon-Carbono industrial.' },
                        { title: 'Post-Procesado', desc: 'Lijado, pintado y ensamblado profesional.' }
                     ].map((item, i) => (
                        <div key={i} className="flex gap-4 items-start group">
                           <div className="w-12 h-12 rounded-full border border-gray-300 dark:border-gray-800 flex items-center justify-center group-hover:bg-teal-500 group-hover:text-white dark:group-hover:text-black group-hover:border-teal-500 transition-all text-gray-400 dark:text-gray-500">
                              <Check className="w-5 h-5" />
                           </div>
                           <div>
                              <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors">{item.title}</h3>
               ...
               <div className="relative h-[600px] bg-gray-100 dark:bg-gray-900 rounded-3xl overflow-hidden group shadow-2xl shadow-gray-200 dark:shadow-none">
                  <img
                     src="https://images.unsplash.com/photo-1623936665077-4952c10b777a?q=80&w=1000&auto=format&fit=crop"
                     className="absolute inset-0 w-full h-full object-cover opacity-80 dark:opacity-60 group-hover:scale-105 transition-transform duration-700"
                  />
                  <div className="absolute bottom-8 left-8 right-8 p-6 bg-white/80 dark:bg-black/50 backdrop-blur-md rounded-2xl border border-white/20 dark:border-white/10 shadow-lg">
                     <div className="text-teal-600 dark:text-teal-400 text-sm font-bold uppercase mb-2">Caso de éxito</div>
                     <h4 className="text-xl font-bold text-gray-900 dark:text-white">Prototipo Funcional - Turbina</h4>
                  </div>
               </div>
            </div>
         </section>
         ...
         <section className="py-20 px-4 max-w-7xl mx-auto">
            <div className="flex items-center justify-center gap-2 mb-12">
               ...
               <h2 className="text-3xl font-bold text-gray-900 dark:text-white">Seguinos en Instagram</h2>
            </div>

            <InstagramFeed />

            <div className="text-center mt-8">
               <a href="https://instagram.com/grana3d" target="_blank" className="inline-flex items-center gap-2 text-teal-600 dark:text-teal-500 hover:text-teal-500 dark:hover:text-teal-400 font-bold transition-colors">
                  @GRANA3D <ArrowRight className="w-4 h-4" />
               </a>
            </div>
         </section>
         ...
         <footer className="border-t border-gray-200 dark:border-gray-900 py-12 px-4 bg-gray-50 dark:bg-black">
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
               ...
               <div className="flex gap-8 text-sm text-gray-500 font-medium">
                  ...
                  <a href="#" className="hover:text-teal-600 dark:hover:text-white transition-colors">INSTAGRAM</a>
                  <a href="#" className="hover:text-teal-600 dark:hover:text-white transition-colors">CONTACTO</a>
               </div>
