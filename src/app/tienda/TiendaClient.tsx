...
interface Producto {
    ...
    imagenes: string[]; categoria: { nombre: string } | null; variantes: any
}
...
function ProductCard({ producto }: { producto: Producto }) {
    ...
    return (
        <motion.div
            variants={fadeIn}
            className="group relative bg-white dark:bg-[#1a1a1a] rounded-xl overflow-hidden border border-gray-100 dark:border-gray-800 hover:border-[#00AE42] dark:hover:border-[#00AE42] transition-all duration-300 flex flex-col h-full"
        >
            ...
            <div className="p-4 flex-1 flex flex-col">
                ...
                {hasVariants && (
                    <div className="flex flex-wrap gap-1 mb-2">
                        {variantGroups.slice(0, 2).map((g: any) => (
                            <span key={g.id} className="text-[9px] text-gray-500 border border-gray-200 dark:border-gray-700 px-1.5 py-0.5 rounded">
                ...
                <div className="flex items-center gap-2 mt-auto">
                    {hasVariants && minPrice !== precio && (
                        <span className="text-[10px] text-gray-500 uppercase font-bold mr-1">Desde</span>
                    )}
                    <span className="text-lg font-bold text-gray-900 dark:text-white">${minPrice.toLocaleString('es-AR')}</span>
                    {tieneOferta && (
